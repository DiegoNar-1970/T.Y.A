...
const ServiceContractPdf = () => {

const { typeContract,dataFormContract } = useSectionContractStore();
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.text}>
            ...
            prestar asesoría jurídica en todo momento al mandante en los siguientes asuntos: {''}
             {typeContract===constTypeContract.LEGAL_SERVICES ? 'Demanda juzgado civil':'Juzgado civil municipal'} en contra de{' '}
            {demandados.map((demandado, index) => (
              <React.Fragment key={index}>
                <Text style={styles.bold}>{demandado.name.toUpperCase()}</Text> con NIT <Text style={styles.bold}>{demandado.typeDoc}</Text>{' '}
