        ...
        {fields.map((field, index) => (
          <div key={field.id} className='flex gap-4 mb-2 justify-around'>
            ...
            <div className='flex flex-col gap-1'>
              <label htmlFor={`${type}-${index}-typeDoc`}>Tipo de Documento</label>
              <select
                {...register(`${type}.${index}.typeDoc`, {
                  required: 'Seleccione un tipo de documento'
                })}
                className={`outline-[1px] rounded-[20px] p-[5px] text-[13px] 
                  hover:outline-[#0d86ffbb] hover:placeholder:text-[#0d86ffbb] 
                  transition-all duration-300 ease-in-out 
                  ${errors[type]?.[index]?.typeDoc ? 'border-red-500 placeholder:text-red-500' : 'border-gray-300'}`}
              >
              ...
              {errors[type]?.[index]?.typeDoc && (
                <p className="text-red-500 text-xs pl-2">
                  {errors[type][index].typeDoc.message}
                </p>
              )}
            </div>

            <div className="flex flex-col gap-1 mb-[-10px]">
              <label htmlFor={`${type}-${index}-doc`}>Documento</label>
              <input
                {...register(`${type}.${index}.doc`, {
                ...
                className={`outline-[1px] rounded-[20px] p-1 text-[13px] 
                  hover:outline-[#0d86ffbb] hover:placeholder:text-[#0d86ffbb] 
                  transition-all duration-300 ease-in-out 
                  ${errors[type]?.[index]?.doc ? 'border-red-500 placeholder:text-red-500' : 'border-gray-300'}`}
              />
              {errors[type]?.[index]?.doc && (
                <p className="text-red-500 text-xs mt-1 pl-2">
                  {errors[type][index].doc.message}
                </p>
              )}
            </div>
